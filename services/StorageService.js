...
class StorageService {
  ...
  async uploadFile(file, metadata = {}) {
    ...
    if (file.path && fs.existsSync(file.path)) {
      ...
      uploadData = {
        buffer,
        filename:
          metadata.filename || file.originalname || `file-${Date.now()}`,
    ...
    console.log(
      `📤 Uploading to ${this.provider}: ${uploadData.filename} (${uploadData.size} bytes)`
    );
  ...
  async uploadToLocal(uploadData) {
    try {
      ...
      const cid = `local_${Buffer.from(uniqueFilename)
        .toString("base64")
        .substring(0, 20)}`;
  ...
  async deleteFromLocal(cid) {
    try {
      ...
      const targetFile = files.find(
        (file) =>
          cid.includes(file) ||
          Buffer.from(file).toString("base64").substring(0, 20) ===
            cid.replace("local_", "")
      );
  ...
  async initializeWeb3Storage() {
    ...
    try {
      ...
      const { Web3Storage } = await import("web3.storage").catch(() => {
        throw new Error(
          "web3.storage package not installed. Run: npm install web3.storage"
        );
      });

      this.config.client = new Web3Storage({
