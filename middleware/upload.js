...
const fileFilter = (req, file, cb) => {
    ...
    if (mimeToExtMap[file.mimetype]) {
        securityChecks.extensionMatches = mimeToExtMap[file.mimetype].includes(fileExt);
    }
    ...
    if (isSecure) {
        ...
        cb(new Error(`Security check failed: ${failedChecks.join(', ')}. Allowed types: ${allowedExtensions.join(', ')}`), false);
    }
};
