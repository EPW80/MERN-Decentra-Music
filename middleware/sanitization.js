...
export const mongoSanitizeMiddleware = mongoSanitize({
    replaceWith: '_',
    onSanitize: ({ req, key }) => {
        console.warn(`⚠️ MongoDB injection attempt detected: ${key} from ${req.ip}`);
    }
});
...
export const validateTrackInput = [
    ...
    body('genre')
        .trim()
        .isIn(['rock', 'pop', 'jazz', 'classical', 'electronic', 'hip-hop', 'country', 'blues', 'reggae', 'folk', 'other'])
...
export const validateSearchInput = [
    ...
    query('genre')
        .optional()
        .trim()
        .isIn(['rock', 'pop', 'jazz', 'classical', 'electronic', 'hip-hop', 'country', 'blues', 'reggae', 'folk', 'other', 'all'])
...
export const validateFileUpload = (req, res, next) => {
    try {
        ...
        console.log(`✅ File validated: ${file.originalname} (${Math.round(file.size / 1024)}KB, ${file.mimetype})`);
        next();
...
export const sanitizeResponse = (req, res, next) => {
    ...
    res.json = function(data) {
        ...
        if (data && typeof data === 'object') {
            ...
            const preserveFields = ['_id', 'id', 'createdAt', 'updatedAt', 'timestamp'];
